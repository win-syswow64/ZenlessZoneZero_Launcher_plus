...
namespace ZenlessZoneZero_Launcher_plus.Server
{
                ...
                if (App.Current.DataModel.Cps != "hoyoverse")
                {
                    switch (value)
                    {
                        case 0:
                            ...
                            if (File.Exists(Path.Combine(App.Current.DataModel.GamePath, "ZenlessZoneZero_Data/Plugins/PCGameSDK.dll")))
                                File.Delete(Path.Combine(App.Current.DataModel.GamePath, "ZenlessZoneZero_Data/Plugins/PCGameSDK.dll"));
                            ...
                            if (!File.Exists(Path.Combine(App.Current.DataModel.GamePath, "ZenlessZoneZero_Data/Plugins/PCGameSDK.dll")))
                            {
                                try
                                {
                                    FileHelper.ExtractEmbededAppResource("StaticRes/mihoyosdk.dll",
                                        Path.Combine(App.Current.DataModel.GamePath, "ZenlessZoneZero_Data/Plugins/PCGameSDK.dll"));
                                }
                                catch (Exception ex)
                                {
